...
class TCProcessor:
    ...
    def extract_tc_from_pdf(self, pdf_bytes):
        """Extrae SOLO Terms & Conditions limpiamente"""
        try:
            ...
            if "Included Services" not in full_text:
                raise Exception("Formato no reconocido")

            start = full_text.find("Included Services")
            extracted = full_text[start:]

            if "By signing this" in extracted:
                end = extracted.find("By signing this")
    ...
    def transform_text(self, text, entity):
        ...
        skip_list = [
            'Industriestraße', 'Geschäftsführer', 'Deutsche Bank', 'IBAN',
            'BIC', 'SWIFT', 'HRB', 'Amtsgericht', 'Josephine', 'Marko',
            'Schkeuditz', 'Heigel', 'sales@', 'www.', 'VAT:', 'DE 365',
            'Charter Agreement', 'Ref. ID', 'DYFKEL', 'Thank you',
            'Citation Sovereign', 'Prepared for', 'Contact Person',
            'Aircraft:', 'Date ETD', 'Arrival at', 'Pax', 'Price',
            'Total Price', 'The shown schedule', 'All times shown'
        ]
        ...
        text = re.sub(r'(\d+)%(?:\s+(?:of|after)\s+charter)', fix_pct, text, flags=re.IGNORECASE)
